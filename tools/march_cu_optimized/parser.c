...
int parseCNF( FILE* in )
{
	...
	*/
	_clause = (int*) SAFE_MALLOC( sizeof( int ) * nrofvars );

	/* INIT GLOBAL DATASTRUCTURES!! */

	Cv = (int**) SAFE_MALLOC( sizeof( int* ) * nrofclauses );
	for( i = 0; i < nrofclauses; i++ )
		Cv[ i ] = NULL;

	/* Clength: length of clause i */
	Clength = (int*) SAFE_MALLOC( sizeof( int ) * nrofclauses );

        /* BinaryImp: implication clause table */
        BinaryImp       = (int**) SAFE_MALLOC( sizeof( int* ) * ( 2 * nrofvars + 1) );
        BinaryImpLength = (int* ) SAFE_MALLOC( sizeof( int  ) * ( 2 * nrofvars + 1) );
	...
	timeAssignments = (tstamp*) SAFE_MALLOC( sizeof( tstamp ) * ( 2 * nrofvars + 1 ) );
	VeqDepends      = (int*   ) SAFE_MALLOC( sizeof( int ) * ( nrofvars + 1 ) );
	...
	while( i < nrofclauses && !error )
	{
		...
		if( _lit == 0 )
		{
			...
			Cv[ i ] = (int*) SAFE_MALLOC( sizeof( int ) * clen );
			Clength[ i ] = clen;
                        if (quiet_mode) {
...
int find_and_propagate_unary_clauses () {
  ...
  simplify_stack = (int*) SAFE_MALLOC(sizeof(int) * (nrofvars + 1));
...
int sort_clauses () {
	...
	{
		clen = Clength[ i ];
		tmpcls = (int*) SAFE_MALLOC( sizeof( int ) * ( clen + 1 ) );
		tmpcls[ 0 ] = clen;
	...
	for( i = 0; i < nrofclauses; i++ )
	{
		...
		{
			tmpcls = (int*) SAFE_MALLOC( sizeof( int ) * clen );
	...
	Cv 	= (int**) SAFE_REALLOC( Cv     , sizeof( int* ) * _nrofclauses );
	Clength = (int* ) SAFE_REALLOC( Clength, sizeof( int  ) * _nrofclauses );
