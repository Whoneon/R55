...
//      printf("   -v            more verbose output\n\n");
  ...
  runParser (argv[1]);
	...
	if (equivalence_reasoning() == UNSAT) handleUNSAT();
        ...
        /* OPT-R55: Disable doublelook for dense k-SAT instances.
           Doublelook iterates over all binary implications for each
           lookahead probe — with density > 100 cls/var (e.g., Ramsey
           encodings at ~2000 cls/var) this becomes a bottleneck.
           Users can still override via -dli on the command line. */
        if (kSAT_flag && nrofvars > 0) {
          int density = nrofclauses / nrofvars;
          if (density > 100 && dl_iter == DL_ITER) {
            dl_iter = 0;
            if (quiet_mode == 0)
              printf("c OPT: doublelook disabled (density=%d cls/var)\n", density);
          }
        }
	...
	necessary_assignments = 0;
        ...
        int* status;
        status = (int*) SAFE_MALLOC(sizeof (int) * (2 * nrofvars + 1));
