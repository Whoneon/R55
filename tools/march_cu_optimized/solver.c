...
int initSolver()
{
	...
	decisions = (int*) SAFE_MALLOC( sizeof(int) * (nrofvars + 1) );

        var_weight    = (int*) SAFE_MALLOC(sizeof(int) * (nrofvars + 1) );
	...
	*/
	...
	tmpEqImpSize  = (int*) SAFE_MALLOC( sizeof(int) * (nrofvars+1) );
	...
	tmpTernaryImpSize = (int* ) SAFE_MALLOC( sizeof(int ) * ( 2*nrofvars+1 ) );
#ifdef TERNARYLOOK
        TernaryImp 	  = (int**) SAFE_MALLOC( sizeof(int*) * ( 2*nrofvars+1 ) );
        TernaryImpSize 	  = (int* ) SAFE_MALLOC( sizeof(int ) * ( 2*nrofvars+1 ) );
	...
	for( i = 0; i < nrofclauses; i++ )
        ...
        for( i = 0; i <= 2 * nrofvars; i++ )
	    TernaryImp[ i ] = (int*) SAFE_MALLOC(sizeof(int)*(4*TernaryImpSize[i]+4));
	...
	/* initialise global datastructures */
	  ...
	  if (kSAT_flag) {
	    ...
	    clause_reduction = (int*) SAFE_MALLOC(sizeof (int) * nrofbigclauses);
	    clause_red_depth = (int*) SAFE_MALLOC(sizeof (int) * nrofbigclauses);
	    big_global_table = (int*) SAFE_MALLOC(sizeof (int) * _nrofliterals );
	    clause_SAT_flag  = (int*) SAFE_MALLOC(sizeof (int) * nrofbigclauses);
        ...
        TernaryImp 	= (int**) SAFE_MALLOC( sizeof(int*) * ( 2*nrofvars+1 ) );
        TernaryImpSize 	= (int* ) SAFE_MALLOC( sizeof(int ) * ( 2*nrofvars+1 ) );
        TernaryImpLast 	= (int* ) SAFE_MALLOC( sizeof(int ) * ( 2*nrofvars+1 ) );

        /* OPT-R55: Size TernaryImpTable based on actual ternary clauses,
           not total clauses.  For pure k-SAT (k>3) instances like Ramsey
           encodings this saves ~46 MB (6*2M ints → 1 int). */
        { int _nrofternary = 0;
          for (i = 0; i < nrofclauses; i++)
            if (Clength[i] == 3) _nrofternary++;
          TernaryImpTable = (int*) SAFE_MALLOC( sizeof(int) * ( 6*_nrofternary+1 ) );
        }
	...
	push_stack_blocks();
        ...
        trailSize = 0;
        trail = (int*) SAFE_MALLOC(sizeof(int) * nrofvars);
