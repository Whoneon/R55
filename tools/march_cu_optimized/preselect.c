...
void init_preselection() {
  ...
  CandidatesSet = (int*  ) SAFE_MALLOC( sizeof( int   ) * ( nrofvars + 1 ) );
  Rank          = (float*) SAFE_MALLOC( sizeof( float ) * ( nrofvars + 1 ) );

  initial_freevars = freevars;

#ifdef HIDIFF
  clause_weight = (float*) SAFE_MALLOC( sizeof(float) * nrofbigclauses );
#endif
...
//#endif
	block_stamp = 0;
    	...
    	freevarsArray  = (int*) SAFE_MALLOC( sizeof( int ) * (nrofvars + 1) );
	freevarsLookup = (int*) SAFE_MALLOC( sizeof( int ) * (nrofvars + 1) );

	block_queue    = (int*) SAFE_MALLOC( sizeof( int ) * (nrofvars + 1) );
	block_clause   = (int*) SAFE_MALLOC( sizeof( int ) * (nrofbigclauses) );
...
*/
	...
	/* OPT-R55: For low-density instances (typical 3-SAT), use full lookahead
	   as in the original code.  For high-density k-SAT (e.g., Ramsey encodings
	   with 2000+ clauses/variable), the adaptive limits above are essential
	   to avoid O(vars * clauses) lookahead at every node. */
	if( kSAT_flag == 0 && (nrofclauses / (nrofvars > 0 ? nrofvars : 1)) <= 10 )
