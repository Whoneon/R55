...
void init_equivalence () {
  int i;
        ...
        Ceq        = (int**) SAFE_MALLOC( sizeof( int* ) * 2 * nrofclauses );
        CeqSizes   = (int*)  SAFE_MALLOC( sizeof( int  ) * 2 * nrofclauses );
        CeqStamps  = (int*)  SAFE_MALLOC( sizeof( int  ) * 2 * nrofclauses );
        CeqValues  = (int*)  SAFE_MALLOC( sizeof( int  ) * 2 * nrofclauses );
        CeqDepends = (int*)  SAFE_MALLOC( sizeof( int  ) * 2 * nrofclauses );
	Veq        = (int**) SAFE_MALLOC( sizeof( int* ) * ( nrofvars + 1 ) );
        VeqLUT     = (int**) SAFE_MALLOC( sizeof( int* ) * ( nrofvars + 1 ) );
        VeqLength  = (int*)  SAFE_MALLOC( sizeof( int  ) * ( nrofvars + 1 ) );

        for( i = 0; i <= nrofvars; i++ )
        {
            Veq        [ i ] = (int*) SAFE_MALLOC( sizeof( int ) * 3 );
            VeqLUT     [ i ] = (int*) SAFE_MALLOC( sizeof( int ) * 3 );
            VeqLength  [ i ] = 2;
            Veq   [ i ][ 0 ] = 1;
            VeqLUT[ i ][ 0 ] = 1;
        }
	...
	eq_found = (int*) SAFE_MALLOC( sizeof(int) * original_nrofvars );
...
void dispose_preprocessor_eq()
{
	...
	Ceq        = (int**) SAFE_REALLOC( Ceq       , sizeof( int* ) * (nrofceq+1) );
	CeqSizes   = (int* ) SAFE_REALLOC( CeqSizes  , sizeof( int  ) * (nrofceq+1) );
	CeqStamps  = (int* ) SAFE_REALLOC( CeqStamps , sizeof( int  ) * (nrofceq+1) );
	CeqValues  = (int* ) SAFE_REALLOC( CeqValues , sizeof( int  ) * (nrofceq+1) );
	CeqDepends = (int* ) SAFE_REALLOC( CeqDepends, sizeof( int  ) * (nrofceq+1) );
}
...
int substitude_equivalences( )
{
        ...
        VeqValues = (int*) SAFE_MALLOC( sizeof( int ) * ( nrofvars + 1 ) );
...
void substitude_ceq( int ceqsource, int ceqtarget )
{
	int k, l, m;
        ...
        Ceq[ ceqsource ] = (int*) SAFE_REALLOC( Ceq[ ceqsource ], sizeof( int ) *
...
int find_equivalence( int clause )
{
	...
	int *lit;
        lit = (int*) SAFE_MALLOC(sizeof(int) * size);
	...
	for( i = 1; i < domain; i++ )
	{
        ...
        Ceq      [ nrofceq ] = (int*) SAFE_MALLOC( sizeof( int ) * size );
...
void replace_bieq( int lit1, int lit2, int ***__Vc, int ***__VcLUT )
{
	...
	for( i = -1; i <= 1; i +=2 )
	{
	        ...
	        VARto = -i * sign * NR( lit2 );
  	      	...
  	      	_Vc[ VARto ] = (int*) SAFE_REALLOC( _Vc[ VARto ],
        		( _Vc[ VARfrom ][ 0 ] + _Vc[ VARto ][ 0 ] + 2 ) * sizeof( int ) );

	        _VcLUT[ VARto ] = (int*) SAFE_REALLOC( _VcLUT[ VARto ],
...
int find_and_propagate_bieq()
{
	...
	*/
	...
	bi_stamp  = (int*) SAFE_MALLOC( sizeof(int) * (nrofvars + 1) );
	bi_target = (int*) SAFE_MALLOC( sizeof(int) * (nrofvars + 1) );
...
void add_binary_equivalence( int lit1, int lit2 )
{
	...
	Ceq      [ nrofceq ]      = (int*) SAFE_MALLOC( sizeof(int) * 2 );
