...
void init_lookahead () {
  ...
  for (i = 0; i < nrofclauses; i++)
	...
	size_diff = (float*) SAFE_MALLOC(sizeof(float) * longest_clause );
	size_diff[ 0 ] = 0.0;
	size_diff[ 1 ] = 0.0;
	size_diff[ 2 ] = h_bin;
	...
	currentTimeStamp = 0;
        ...
        lookaheadArray = (int*) SAFE_MALLOC( sizeof( int ) * 2 * nrofvars );
...
#endif
	forced_literal_array = (int*)  SAFE_MALLOC( sizeof(int) * (nrofvars+1) );
	treeArray = (struct treeNode*) SAFE_MALLOC( sizeof(struct treeNode) * (2*nrofvars+1) );
	...
	diff_table = (float* ) SAFE_MALLOC( sizeof(float ) * 100 * (2*nrofvars+1) );
	diff_depth = (float**) SAFE_MALLOC( sizeof(float*) * 100                  );
	for( i = 0; i < 100; i++ )
	...
	if (non_tautological_equivalences) {
       	    lengthWeight = (double *) SAFE_MALLOC(sizeof(double) * (100000));
	    lengthWeight[ 0 ] = 0.0;
	    lengthWeight[ 1 ] = 0.0;
...
void ternarylook_detect_constraint_resolvents( const int lit1, const int lit2 )
{
	if( !ternarylook_add_constraint_resolvents( lit1, lit2 ) )
	{
	    ...
	    Cv                = (int**) SAFE_REALLOC( Cv,      sizeof( int*) * ( nrofclauses + 1 ) );
	    Cv[ nrofclauses ] = (int* ) SAFE_MALLOC( sizeof(int) * 3);
	    Clength           = (int* ) SAFE_REALLOC( Clength, sizeof( int ) * ( nrofclauses + 1 ) );
...
int ternarylook_add_constraint_resolvents( const int lit1, const int lit2 )
{
	...
	*(look_fixstackp++) = lit2;
	...
	while( _look_fixstackp < look_fixstackp )
        ...
        Cv      = (int**) SAFE_REALLOC( Cv,      sizeof( int*) * ( nrofclauses + (look_resstackp - _look_resstackp ) ) );
        Clength = (int *) SAFE_REALLOC( Clength, sizeof( int ) * ( nrofclauses + (look_resstackp - _look_resstackp ) ) );
	...
	while( _look_resstackp < look_resstackp )
	{
		...
		if( tmpTernaryImpSize[ clause[i] ] <= (2 * TernaryImpSize[ clause[i] ])  )
		{
		    ...
		    TernaryImp[ clause[i] ] = (int*) SAFE_REALLOC( TernaryImp[ clause[i] ],
