...
void add_resolvent () {
  if (nrofresolvents == resolvent_list_size) {
    ...
    resolvent_list = (int**) SAFE_REALLOC(resolvent_list, sizeof(int*) * resolvent_list_size);
    resolvent_size = (int* ) SAFE_REALLOC(resolvent_size, sizeof(int ) * resolvent_list_size); }

  int size = new_resolvent_end - new_resolvent_begin;
  resolvent_size[ nrofresolvents ] = size;
  resolvent_list[ nrofresolvents ] = (int*) SAFE_MALLOC(sizeof(int) * size);
...
void make_resolvents()
{
	...
	resolvent_list = (int**) SAFE_MALLOC( sizeof(int*) * resolvent_list_size );
	resolvent_size = (int* ) SAFE_MALLOC( sizeof(int ) * resolvent_list_size );

	stamps  = (int*) SAFE_MALLOC( sizeof(int) * nrofvars );

	new_resolvent_begin = (int*) SAFE_MALLOC( sizeof(int) * nrofvars );
	new_resolvent_end   = new_resolvent_begin;
	...
	Cv      = (int**) SAFE_REALLOC( Cv     , sizeof(int*) * (nrofclauses + nrofresolvents) );
	Clength = (int* ) SAFE_REALLOC( Clength, sizeof(int ) * (nrofclauses + nrofresolvents) );
...
void detect_resolvents()
{
	...
	rstack  = (int*) SAFE_MALLOC( sizeof(int) * nrofvars );
	rstackp = rstack;
