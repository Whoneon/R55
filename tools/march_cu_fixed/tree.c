...
void init_tree()
{
	...
	NodeType = (int *) SAFE_MALLOC(sizeof(int)*(2*nrofvars+1));
	for(i = 0; i < 2* nrofvars + 1; i++ ) NodeType[ i ] = USEDNODE;
	NodeType += nrofvars;

        assignment_array = (struct assignment *)SAFE_MALLOC(sizeof(struct assignment)*(nrofvars+1)*2);
            ...
            for( j = 0; j < 2; j++ )
        {
            ...
            assignment->incoming = (int *)SAFE_MALLOC( 16 * sizeof(int));
        }

/* ================== group SG4 ======================= */
	fai_  = (int *) SAFE_MALLOC(sizeof(int)*(2*nrofvars+1));
	for(i = 0; i < 2* nrofvars + 1; i++ ) fai_ [ i ] = 0 ;

	comp_  = (int *) SAFE_MALLOC(sizeof(int)*(2*nrofvars+1));
	for(i = 0; i < 2* nrofvars + 1; i++ ) comp_ [ i ] = 0 ;

	vstack  = (int *) SAFE_MALLOC(sizeof(int)*(2*nrofvars+1));
