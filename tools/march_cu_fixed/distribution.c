...
void init_direction()
{
	...
	forced_array = (int*) SAFE_MALLOC( sizeof(int) * forced_array_size );

	record_array_size = INITIAL_RECORD_ARRAY_SIZE;
	records = (struct record*) SAFE_MALLOC( sizeof(struct record) * record_array_size );
...
int init_new_record()
{
	...
	if( record_array_size <= recorded_nodes )
	{
	    ...
	    records = (struct record*) SAFE_REALLOC( records, sizeof(struct record) * record_array_size );
	}
...
void check_forced_array_capacity()
{
	if( forced_elements + freevars > forced_array_size )
	{
	    ...
	    forced_array = (int*) SAFE_REALLOC( forced_array, sizeof(int) * forced_array_size );
	}
}
...
void fill_distribution_bins()
{
	...
	bins = (int*) SAFE_MALLOC( sizeof(int) * (1<<CUT_OFF) );
