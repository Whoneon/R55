...
{ \
        ...
        if (_a##stack == NULL) { fprintf(stderr, "FATAL: INIT_ARRAY malloc failed\n"); exit(1); } \
        _a##stackp    = _a##stack; \
}
...
\
        ...
        if (_a == NULL) { fprintf(stderr, "FATAL: MALLOC_OFFSET malloc failed\n"); exit(1); } \
...
#define PUSH( stk, value ) \
	...
	{ \
		...
		__newp = realloc( stk##stack, stk##stackSize*sizeof(int)); \
		if (__newp == NULL) { fprintf(stderr, "FATAL: PUSH realloc(%d) failed\n", stk##stackSize); exit(1); } \
		stk##stack = (int *) __newp; \
...
{ \
        ...
        { \
             void *__p = realloc( BinaryImp[ ic ], sizeof( int ) * 2 * ( BinaryImpLength[ ic ] + 1 ) ); \
             if (__p == NULL) { fprintf(stderr, "FATAL: CHECK_BIMP_BOUND realloc failed\n"); exit(1); } \
             BinaryImp[ ic ] = (int*) __p; \
...
{ \
        ...
        { \
             void *__p = realloc( BinaryImp[ ic ], sizeof( int ) * (2 * BinaryImpLength[ ic ] + size + 1) ); \
             if (__p == NULL) { fprintf(stderr, "FATAL: CHECK_BIMP_UPPERBOUND realloc failed\n"); exit(1); } \
             BinaryImp[ ic ] = (int*) __p; \
...
{ \
        ...
        { \
             void *__p = realloc( BinaryImp[ ic ], sizeof( int ) * (2 * BinaryImpLength[ ic ] + size + 1) ); \
             if (__p == NULL) { fprintf(stderr, "FATAL: CHECK_BIMP_DOUBLEBOUND realloc failed\n"); exit(1); } \
             BinaryImp[ ic ] = (int*) __p; \
             BinaryImpLength[ ic ] += BinaryImpLength[ ic ] + 1 + size; \
        }\
}
...
{ \
        ...
        { \
             ...
             void *__p1 = realloc( Veq[ vq ], sizeof( int ) * VeqLength[ vq ]  ); \
             void *__p2 = realloc( VeqLUT[ vq ], sizeof( int ) * VeqLength[ vq ]  ); \
             if (!__p1 || !__p2) { fprintf(stderr, "FATAL: CHECK_VEQ_BOUND realloc failed\n"); exit(1); } \
             Veq[ vq ] = (int*) __p1; \
             VeqLUT[ vq ] = (int*) __p2; \
        }\
}
