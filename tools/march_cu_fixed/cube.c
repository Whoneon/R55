...
void Dnode_init (int index) {
  ...
  if (nrofDnodes + 3 > Dnodes_size) {
    Dnodes = (struct Dnode*) SAFE_REALLOC(Dnodes, sizeof(struct Dnode) * Dnodes_size * 2);
...
void init_assumptions () {
  nrofDnodes  =   1;
  Dnodes_size = 100;
  Dnodes = (struct Dnode*) SAFE_MALLOC(sizeof(struct Dnode) * Dnodes_size);
...
void filterTree (int limit) {
  int i;
  int *filter = (int*) SAFE_MALLOC(sizeof(int) * limit);
  filter[0] = 1;
...
void printDecisionTree () {
  ...
  cubeTrail = (int*) SAFE_MALLOC(sizeof(int) * nrofvars);
