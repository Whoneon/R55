...
void allocate_big_clauses_datastructures()
{
	int i, j, nrofliterals = 0, lit, tmp;
        ...
        float* wlit = (float *) SAFE_MALLOC(sizeof(float) * (2*nrofvars + 1));
        wlit += nrofvars;
...
//          float w = pow (1000.0 * Clength[i], 0.5);
        ...
        clause_weight   = (float*) SAFE_MALLOC( sizeof(float) * nrofclauses );
...
//          printf("%.5f ", clause_weight[i]);
        ...
        big_occ = (int*) SAFE_MALLOC( sizeof(int) * (2*nrofvars + 1) );
        big_occ += nrofvars;
        for( i = 1; i <= nrofvars; i++ )
        ...
        literal_list    = (int*  ) SAFE_MALLOC( sizeof(int  ) * (nrofliterals + nrofbigclauses) );
        clause_list     = (int** ) SAFE_MALLOC( sizeof(int* ) * nrofbigclauses );
        clause_length   = (int*  ) SAFE_MALLOC( sizeof(int  ) * nrofbigclauses );
        clause_database = (int*  ) SAFE_MALLOC( sizeof(int  ) * (nrofliterals + 2 * nrofvars) );
        ...
        clause_set  = (int**) SAFE_MALLOC( sizeof(int*) * (2*nrofvars + 1) );
        clause_set += nrofvars;
...
void allocateTernaryImp( int **_tImpTable, int ***_tImp, int **_tImpSize )
{
	...
	int *_TernaryImpTable, **_TernaryImp, *_TernaryImpSize;
        ...
        _TernaryImpSize  = (int* ) SAFE_MALLOC( sizeof(int ) * ( 2 * nrofvars + 1 ) );
	...
	_TernaryImpTable = (int* ) SAFE_MALLOC( sizeof(int ) * ( 6 * _nrofternaryclauses ) );
        _TernaryImp      = (int**) SAFE_MALLOC( sizeof(int*) * ( 2 * nrofvars + 1 ) );
...
void allocateVc( int ***_Vc, int ***_VcLUT )
{
        ...
        */
        __Vc     = (int**) SAFE_MALLOC( sizeof( int*) * ( 2 * nrofvars + 1 ) );
        __VcLUT  = (int**) SAFE_MALLOC( sizeof( int*) * ( 2 * nrofvars + 1 ) );
        __VcTemp = (int* ) SAFE_MALLOC( sizeof( int ) * ( 2 * nrofvars + 1 ) );
        ...
        */
        ...
        {
                __Vc   [ i ] = (int*) SAFE_MALLOC( sizeof( int ) * __VcTemp[ i ]  );
                __VcLUT[ i ] = (int*) SAFE_MALLOC( sizeof( int ) * __VcTemp[ i ] );
...
void resize_BinaryImp()
{
        ...
        BinaryImp       = (int**) SAFE_REALLOC( BinaryImp,       sizeof(int*) * ( 2 * nrofvars + 1) );
        BinaryImpLength = (int* ) SAFE_REALLOC( BinaryImpLength, sizeof(int ) * ( 2 * nrofvars + 1) );

        for( i = 0; i < (2 * nrofvars + 1); i++ )
        {
                BinaryImp      [ i ] = (int*) SAFE_MALLOC( sizeof( int ) * INITIAL_ARRAY_SIZE );
